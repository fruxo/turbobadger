...
/** TBBFRenderer renders a bitmap font.
	...
	Text file format (in tb.txt format parsed by tinkerbell/parser/TBParser.h):
		...
		- info>rgb				Set to 1 for color fonts that should never
								...
								color. Default 0.
		- size_xx				Specify font size xx. Should contain the
								following nodes:
...
bool TBBFRenderer::Load(const char *filename, int size)
{
	...
	TBStr request;
	request.SetFormatted("size_%d", m_size);
	if (TBNode *n = m_node.GetNode(request))
	{
		// Metrics
		m_metrics.ascent = n->GetValueInt("ascent", 0);
		m_metrics.descent = n->GetValueInt("descent", 0);
		m_metrics.height = m_metrics.ascent + m_metrics.descent;

		// Other data
		m_advance_delta = n->GetValueInt("advance_delta", 0);
		m_space_advance = n->GetValueInt("space_advance", 0);
		m_x_ofs = n->GetValueInt("x_ofs", 0);
	}
	...
	request.SetFormatted("size_%d>bitmap", size);
	bitmap_filename.Append(m_node.GetValueString(request, ""));
